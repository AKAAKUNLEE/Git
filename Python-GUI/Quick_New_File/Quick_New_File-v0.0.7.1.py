...
def update_extension_menu():
    menu = extension_option_menu['menu']
    menu.delete(0, 'end')
    for ext in extensions:
        menu.add_command(label=ext, command=lambda value=ext: set_extension(value))

def update_template_menu(extension):
    menu = template_option_menu['menu']
    menu.delete(0, 'end')
    if extension in extensions:
        for template in extensions[extension]:
            menu.add_command(label=template["name"], command=lambda value=template: set_template(value))

# 设置后缀名并更新文件内容
def set_extension(extension):
    extension_var.set(extension)
    update_template_menu(extension)
    if extensions[extension]:
        set_template(extensions[extension][0])  # 默认选择第一个模板

# 设置模板并更新文件内容
def set_template(template):
    template_var.set(template["name"])
    update_file_content(template["content"])
...
# 初始化内容模板
default_extensions = {
    ".txt": [{"name": "默认文本", "content": "这是默认的文本文件内容。"}],
    ".py": [{"name": "Python脚本", "content": "print('Hello, World!')"}],
    ".json": [{"name": "JSON数据", "content": "{\"key\": \"value\"}"}],
    # 可以根据需要添加更多模板
}

# 合并默认模板和自定义模板
for ext, templates in default_extensions.items():
    if ext not in extensions:
        extensions[ext] = templates

# 初始化内容
if extensions:
    default_value = list(extensions.keys())[0]
else:
    default_value = ".txt"
    extensions[default_value] = [{"name": "默认文本", "content": "这是默认的文本文件内容。"}]

extension_var = tk.StringVar(value=default_value)
if extensions[default_value]:
    default_template = extensions[default_value][0]["name"]
else:
    default_template = "默认文本"

template_var = tk.StringVar(value=default_template)
...
label1 = ttk.Label(page1, text="请选择文件扩展名：", wraplength=110)
label1.grid(row=1, column=0, sticky="w")

# 创建扩展名 OptionMenu 并初始化
extension_option_menu = ttk.OptionMenu(page1, extension_var, default_value, *extensions.keys(), command=set_extension)
extension_option_menu.grid(row=1, column=1, sticky="w")

# 创建模板 OptionMenu 并初始化
template_option_menu = ttk.OptionMenu(page1, template_var, default_template, command=lambda value: set_template(value))
template_option_menu.grid(row=1, column=2, sticky="w")
...
# 初始化文件内容
update_file_content(extensions[default_value][0]["content"])

# 更新 OptionMenu 菜单项
update_extension_menu()
update_template_menu(default_value)
