...
# 页面1的内容
label = ttk.Label(page1, text="请输入文件路径（不包括扩展名）：")
label.grid(row=0, column=0, pady=10, sticky="w")

entry = ttk.Entry(page1, width=50)
entry.grid(row=1, column=0, pady=10, sticky="w")

select_directory_button = ttk.Button(page1, text="选择目录", command=select_directory)
select_directory_button.grid(row=2, column=0, pady=5, sticky="w")
...
option_menu.grid(row=3, column=0, pady=10, sticky="w")

create_button = ttk.Button(page1, text="创建文件", command=create_file)
create_button.grid(row=4, column=0, pady=10, sticky="w")
...
list_button = ttk.Button(page2, text="选择目录", command=list_files)
list_button.grid(row=1, column=0, pady=5, sticky="w")
